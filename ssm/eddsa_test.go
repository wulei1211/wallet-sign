...
func TestCreateEdDSAKeyPair(t *testing.T) {
	var encryption = EncryptionMap["eddsa"]

	privKey, pubKey, _, _ := encryption.CreateKeyPair()
...
func TestSignEdDSAMessage(t *testing.T) {
	...
	txMsg, _ := encryption.SignMessage(privKey, common.Hash{}.String())
...
func TestSignEdDSAMessageV2(t *testing.T) {
	...
	signature, err := encryption.SignMessage(fullPrivateKey, message)
...
func TestVerifyEdDSASign(t *testing.T) {
	...
	ok, _ := encryption.VerifySignature(pubKey, common.Hash{}.String(), signature)
	fmt.Println("ok=", ok)
}
